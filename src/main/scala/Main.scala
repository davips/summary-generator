  ...
  for (f <- 5 to 6) {
    val qsd = List(4, 7, 10) map { side =>
      def parse(arq: String) = {
        ...
        val file = s"f$f-0-${side}x$side-100-$arq"
        val content = Source.fromFile("../ocean/" + file).getLines().toList
        val lines = content.map { l => l.split('\t').toList }

        val (vars, errors) = content.takeRight(10).map(x => x.split('\t').toList.tail).map {
          case List(a, b) if (a + b) forall Character.isDigit => a.toDouble -> b.toDouble
          case _ =>
            def extract(pos: Int) = {
      ...
      val (iniv, oriv, endvsw, inie, orie, endesw) = parse("sw-1.off.log")
      val (_, _, endv1c, _, _, ende1c) = parse("1c-1.off.log")
